...
public partial class employees : System.Web.UI.Page
{
    ...
    {
        get { return ViewState["SortColumn"] != null ? ViewState["SortColumn"].ToString() : "EmployeeID"; }
    ...
    private void BindGrid(string sortExpression = null, bool sort = false, bool where = false, string searchquery = null)  //Called to initially bind and display table on webpage with no sorting string by default || Search string is used to look for specific items with where clause statement
    {
        ...
        using (SqlConnection con = new SqlConnection(constr))  //Binds connection string to sql connection
        {
            ...
            string sqlquery = "SELECT Employees.*, DepartmentName FROM Employees JOIN Departments ON Departments.DepartmentID = Employees.DepartmentID ";  //The default query statement
    ...
    */
    ...
    protected void EmployeeNameSearch(object sender, EventArgs e)
    {
        ...
        {
            this.BindGrid(this.SortColumn, false, true, " CONCAT(FirstName, ' ', LastName) LIKE '%" + employeenametxt.Text + "%'");
        }
    ...
    //Method called when searching for ID of an Employee
    protected void EmployeeIDSearch(object sender, EventArgs e)
    {
        ...
        {
            this.BindGrid(this.SortColumn, false, true, "EmployeeID= 'E-" + employeeidtxt.Text + "'");  //Automatically will have the characters E- for convience
        }
        else  //If the textbox is empty and the submit button is pressed it just refreshes the table. also sends true statement in order to prevent sorting.
        {
            this.WhereClause = null;
    ...
    //Method called when searching for given department name
    ...
    protected void Check_Clicked(object sender, EventArgs e)  //Method called when any member of the checkboxlist is updated and will refresh the table to see if any columns need to be hidden or shown.
    {
        foreach (ListItem item in ColumnCheckBoxList.Items)
        {
            ...
            if(Int32.Parse(item.Value) == 6)  //If state to check if the Mail info group was selected and if so will either hide or show all mail related columns
            {
                for(int i = 6; i < 10; i++)  
                {
                    EmployeeGridView.Columns[i].Visible = item.Selected;
                }
            }
            else if(Int32.Parse(item.Value) == 11)  //If statement checks if the department info ground was selected and will either hide or show all department related columns
            {
                EmployeeGridView.Columns[11].Visible = item.Selected;
                EmployeeGridView.Columns[12].Visible = item.Selected;
            }
